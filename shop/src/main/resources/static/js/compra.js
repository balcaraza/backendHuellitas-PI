...
function leerAPagar() {
  // Obtener el valor del espacio "aPagar" del Local Storage
  const datosJSON = localStorage.getItem('aPagar');
  ...
  if (datosJSON) {
      ...
      // Si no hay datos almacenados, devolver null o un valor predeterminado según lo necesites
      return null;
  }
}

// Obtener los datos del Local Storage
const datosAPagar = leerAPagar();

// Verificar si hay datos para imprimir
if (datosAPagar) {
  // Crear una cadena con los valores a imprimir
  const textoHTML = `${datosAPagar.item2InnerHTML}<br>${datosAPagar.item3InnerHTML}<br>${datosAPagar.item4InnerHTML}`;
  
  // Obtener el elemento <h4> con el id "costoPedido"
  const costoPedidoElemento = document.getElementById('costoPedido');

  // Insertar la cadena en el elemento <h4>
  costoPedidoElemento.innerHTML = textoHTML;
} else {
  // Si no hay datos almacenados, mostrar un mensaje en el elemento <h4>
  const costoPedidoElemento = document.getElementById('costoPedido');
  costoPedidoElemento.innerHTML = "No hay datos para mostrar";
}
/*function leerAPagar() {
  const datosJSON = localStorage.getItem('aPagar');
  ...
  if (datosJSON) {
      const datos = JSON.parse(datosJSON);
      let innerHTML=`<h5 class="styPrecio">${datos.subtotal}</h5>`;
      costoPedido=insertAdjacentHTML("beforeend", innerHTML);
